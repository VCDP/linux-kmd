...
#define MEMIRQ_DEBUG(_gt, ...) \
	DRM_DEV_DEBUG_DRIVER((_gt)->i915->drm.dev, "IRQ " __VA_ARGS__)
#else
#define MEMIRQ_DEBUG(...)
#endif
...
static int vf_create_memirq_data(struct intel_iov *iov)
{
	...
	obj = intel_gt_object_create_lmem(gt, SZ_4K,
...
static int __guc_action_register_memory_irq(struct intel_guc *guc,
					    u32 source, u32 status)
{
	u32 action[] = {
		INTEL_GUC_ACTION_REGISTER_MEMORY_IRQ,
		source,
		status + ilog2(GUC_INTR_GUC2HOST),
	};

	/* Can't use generic send(), IRQ registration must go over MMIO */
	/* XXX: GuC 34.0.0 requires this action over regular CTB */
...
int intel_iov_memirq_prepare_guc(struct intel_iov *iov)
{
	...
	err = __guc_action_register_memory_irq(guc, source, status);
	if (unlikely(err))
		IOV_ERROR(iov, "Failed to register MEMIRQ %#x:%#x (%d)\n",
			  source, status, err);
...
void intel_iov_memirq_handler(struct intel_iov *iov)
{
	...
	intel_presi_flush_memory(iov_to_i915(iov));
