...
extern const struct intel_lmtt_ops ats_lmtt_ops;
...
struct intel_lmtt_ops {
	unsigned int (*root_pd_level)(void);
	unsigned int (*pte_num)(unsigned int level);
	resource_size_t (*pte_size)(unsigned int level);
	unsigned int (*pte_shift)(unsigned int level);
	unsigned int (*pte_idx)(u64 addr, unsigned int level);
	u64 (*pte_encode)(unsigned long offset, u32 flags, unsigned int level);
	void (*pte_write)(void *pt_vaddr, u64 value, unsigned int n,
			  unsigned int level);
	struct intel_lmtt_pt *(*leaf_pt)(struct intel_lmtt *lmtt, u64 addr,
					 unsigned int vf);
};
