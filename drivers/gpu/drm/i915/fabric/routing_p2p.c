...
static u32 bw_from_speed(u32 link_speed)
{
	...
	case LINK_SPEED_50G:  return 531;
	case LINK_SPEED_100G: return 900;
	}
...
static u32 get_bandwidth_link(struct fport *port)
{
	...
	u32 bw_b = get_bandwidth_port(port->cached_neighbor);
 ...
 * @sd_egress: The on-plane subdevice into which traffic will ingress into the
 * destination package in order to reach the destination subdevice
			   ...
			   struct fport *port)
{
	...
	cost_before = routing_cost_lookup(path->sd_curr, trace->sd_egress);
	cost_after = routing_cost_lookup(sd_neighbor, trace->sd_egress);
	...
	if (path->sd_curr == trace->sd_src) {
		 ...
		 */
		port->cached_neighbor = port_neighbor;
	}
	...
	if (path->sd_curr == trace->sd_egress) {
		path->done = true;
		 ...
		 */
		port_neighbor->cached_neighbor = port;
	}
...
static void extend_path(struct trace *trace, struct trace_path *path)
{
	...
	u64 mask_remaining = path->dfid_mask;
	 ...
	 */
	for_each_fabric_lpn(lpn, path_curr->sd_curr) {
		port = get_fport_handle(path_curr->sd_curr, lpn);
		if (!port)
			continue;

		/* determine the set of FIDs that use this port */
		mask_port = 0;
		for_each_set_bit(fid_offset, (void *)&mask_remaining, 64) {
			if (routing_uft_entry_get(block, fid_offset) == lpn)
...
static struct fsubdev *find_egress(struct fsubdev *src, struct fsubdev *dst)
{
	...
	u16 dfid = src->routing.fidgen->map[dpa] << ROUTING_DPA_DFID_MAP_SHIFT;
	int i;
...
static u16 reduce_bandwidth(struct trace *trace)
{
	...
	for_each_set_bit(lpn, trace->port_mask_egress, PORT_COUNT) {
		port = get_fport_handle(trace->sd_egress, lpn);
		 ...
		 struct sd2sd_info *si)
{
	...
	sd_egress = find_egress(sd_src, sd_dst);
	if (!sd_egress)
		return -EINVAL;

	if (routing_cost_lookup(sd_src, sd_egress) == ROUTING_COST_INFINITE)
		return -EINVAL;
		   ...
		   struct sd2sd_info *si_forward)
{
	...
	if (trace(src, dst, si_forward))
		goto zero;

	if (trace(dst, src, &si_reverse))
		goto zero;

	union_sd2sd(si_forward, &si_reverse);
	return;

zero:
	si_forward->bandwidth = 0;
	si_forward->latency = 0;
}
...
static int p2p_entry_alloc_cb(struct fdev *dev, void *args)
{
	...
	if (dev->p2p_next) {
		...
		rcu_swap_protected(dev->p2p, dev->p2p_next, true);
	}
 ...
 * @s: the seq interface
 * @pos: the read position being reqeusted
 *
...
static void *debugfs_p2p_seq_next(struct seq_file *s, void *v, loff_t *pos)
{
	...
	iter = debugfs_p2p_iter_inc(iter);
	if (iter)
		(*pos)++;
...
void routing_p2p_init(void)
{
	debugfs_add_file_node(DEBUGFS_NAME_P2P, 0400, debugfs_get_root_node(),
			      NULL, &debugfs_ops);
}
