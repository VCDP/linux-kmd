...
void rem_init(void)
{
	struct dentry *root_node = debugfs_get_root_node();
	...
	debugfs_add_u32_node("rem_state", 0400, root_node, &rem_state);
	debugfs_add_u64_node("rem_pending_requests", 0400, root_node,
			     &rem_pending_requests);
	debugfs_add_u64_node("rem_serviced_requests", 0400, root_node,
			     &rem_serviced_requests);
	debugfs_add_ulong_node("rem_last_route_jiffies", 0400, root_node,
...
void rem_request(void)
{
	...
	++rem_pending_requests;
	if (rem_state == REM_STATE_IDLE && first)
		rem_schedule();
