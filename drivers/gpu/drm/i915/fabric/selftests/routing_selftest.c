...
#include "selftests/routing_selftest.h"
			       ...
			       struct fsubdev *sd_src)
{
	...
	for (i = start; i < end; ++i) {
		...
		dfid = (u16)(sd_src->routing.fidgen_next->map[i] <<
		...
		TEST(dfid == 0,
		     ...
		     (u16)sd_src->routing.fidgen_next->map[i]);
	}
			   ...
			   struct fsubdev *sd_src)
{
	...
	TEST(sd_src->routing.fidgen_next, "the sd received a fidgen map");
	...
	TEST(!IS_ERR_OR_NULL(block), "sd uft block to self is valid");

	for (dfid = 0; dfid < ROUTING_FID_BLOCK_SIZE; ++dfid) {
		lpn = routing_uft_entry_get(block, dfid);

		if (is_host_fid(dfid)) {
			TEST(lpn == ROUTING_UFT_INVALID_PORT4,
			     "routes to 'host' fids are invalid: device %u sd %u dfid 0x%04x port %u",
			     sd_src->fdev->pd->index, sd_index(sd_src),
			     dfid, lpn);
		...
		TEST(fid_channel == port_channel,
		     "fid mdif channel is consistent with bridge port");
	}
	...
	for_each_fabric_lpn(lpn, sd_src) {
		...
		TEST(!port->portinfo->neighbor_guid || neighbor_of(port),
		     "ports with neighbor guids have neighbors: device %u sd %u port %u neighbor_guid 0x%016llx neighbor 0x%016llx",
		     sd_src->fdev->pd->index, sd_index(sd_src),
		     port->lpn, port->portinfo->neighbor_guid,
		     (u64)neighbor_of(port));
			      ...
			      u16 dfid)
{
	...
	TEST(!IS_ERR_OR_NULL(block),
	     "sd uft block to destination is valid");

	lpn = routing_uft_entry_get(block, dfid - sd_dst->routing.fid_base);
				   ...
				   struct fsubdev *sd_dst, u16 dpa_idx)
{
	...
	u16 dfid = (u16)(sd_src->routing.fidgen_next->map[dpa_idx] <<
			 ROUTING_DPA_DFID_MAP_SHIFT);
	int i;
