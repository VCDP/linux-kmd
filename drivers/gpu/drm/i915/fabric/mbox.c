...
static u64 mbox_build_rsp_cw(u64 cw, u64 req_cw, int rsp_result)
{
	...
	if (rsp_result) {
		...
		rsp_cw |= (rsp_result & MBOX_CW_RSP_STATUS_MASK) <<
			   MBOX_CW_RSP_STATUS_SHIFT;
	}
					   ...
					   int *op_result)
{
	...
	ibox = mbdb_op_build_cw_and_acquire_ibox(sd, mbdb_mbox_op_code(req_cw),
						 mbdb_mbox_params_len(req_cw),
						 recv_data, recv_len, cw,
						 NULL, NULL, posted);
	if (IS_ERR(ibox)) {
		ret = PTR_ERR(ibox);
		kfree(rsp);
...
/**
 * mbox_term - Unregisters the interface between kernel and user space.
 */
void mbox_term(void)
{
...
/**
 * mbox_init - Registers the interface between kernel and user space.
 *
