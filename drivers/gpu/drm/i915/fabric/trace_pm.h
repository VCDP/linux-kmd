...
#define PORT_STATE_CHANGE_PRN "T%uP%d: type %u LMA %u PPS %x OLDRNNLQI %x " \
...
DECLARE_EVENT_CLASS(iaf_pm_template,
		    ...
		    TP_ARGS(sd_id, port_num, portinfo),
		    TP_STRUCT__entry(__field(u16, sd_id)
				     __field(u8, port_num)
		    ...
		    TP_fast_assign(__entry->sd_id = sd_id;
...
DEFINE_EVENT(iaf_pm_template, pm_psc,
	     TP_PROTO(u16 sd_id, u8 port_num, struct portinfo *portinfo),
	     TP_ARGS(sd_id, port_num, portinfo)
	    );
