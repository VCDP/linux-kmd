...
u8 *routing_uft_block_alloc(size_t num_fids)
{
	size_t len = (num_fids + 1) / 2;
	u8 *block = kmalloc(len, GFP_KERNEL);
 ...
 *
 * Return: The requested block if found, otherwise an ERR_PTR value.
 */
 ...
 * bridge blocks.
 *
 * Return: The allocated unicast forwarding table.
 */
...
{
	struct routing_fidgen *prev = sd->routing.fidgen;

	sd->routing.fidgen = sd->routing.fidgen_next;
	sd->routing.fidgen_next = NULL;
	kfree(prev);
}

/**
 * routing_sd - Transitions the specified sd into an error
...
void routing_sd_transition_error(struct fsubdev *sd)
{
	...
	kfree(sd->routing.fidgen);
	sd->routing.fidgen = NULL;
